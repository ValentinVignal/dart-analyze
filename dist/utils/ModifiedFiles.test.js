    ...
    describe('ModifiedFiles class', () => {
        describe('constructor and initialization', () => {
            ...
            it('should initialize files after construction', async () => {
                ...
                expect(modifiedFiles.has('/github/workspace/lib/test1.dart')).toBe(true);
                expect(modifiedFiles.has('/github/workspace/lib/test2.dart')).toBe(true);
                expect(modifiedFiles.has('/github/workspace/lib/test3.dart')).toBe(true);
        ...
        describe('getGithubFiles method', () => {
            ...
            it('should fail for unsupported events', async () => {
                ...
                const modifiedFiles = new ModifiedFiles(mockActionOptions);
                await modifiedFiles.isInit;
                expect(core.setFailed).toHaveBeenCalledWith(expect.stringContaining('This action only supports pull requests and pushes'));
            });
            it('should fail when API returns non-200 status', async () => {
                ...
                const modifiedFiles = new ModifiedFiles(mockActionOptions);
                await modifiedFiles.isInit;
                expect(core.setFailed).toHaveBeenCalledWith(expect.stringContaining('returned 404, expected 200'));
            });
        });
        describe('has method', () => {
            beforeEach(async () => {
            ...
            it('should return false for non-existing files', async () => {
                const modifiedFiles = new ModifiedFiles(mockActionOptions);
                await modifiedFiles.isInit;
                expect(modifiedFiles.has('/github/workspace/lib/nonexistent.dart')).toBe(false);
...
//# sourceMappingURL=ModifiedFiles.test.js.map