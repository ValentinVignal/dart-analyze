...
describe('getInputSafe', () => {
    let getInputSpy;
    beforeEach(() => {
        getInputSpy = vi.spyOn(core, 'getInput');
    });
    afterEach(() => {
    ...
    it('returns value for direct input name', () => {
        getInputSpy.mockImplementation((name) => name === 'emojis' ? 'true' : '');
        expect(getInputSafe('emojis')).toBe('true');
    });
    it('returns value for dash input from github action', () => {
        getInputSpy.mockImplementation((name) => name === 'fail-on' ? 'warning' : '');
    ...
    it('throws if required and not supplied (no dash)', () => {
        ...
        expect(() => getInputSafe('token', { required: true })).toThrowError('Input required and not supplied: token');
    });
    it('throws if required and not supplied (with dash)', () => {
        getInputSpy.mockImplementation(() => '');
        expect(() => getInputSafe('fail-on', { required: true })).toThrowError('Input required and not supplied: fail-on');
    });
...
//# sourceMappingURL=getInput.test.js.map