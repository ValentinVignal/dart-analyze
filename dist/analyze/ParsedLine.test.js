...
const baseLine = 'ERROR|LINT|PREFER_CONST_CONSTRUCTORS|/home/runner/work/repo/lib/main.dart|96|13|80|Prefer const with constant constructors.';
const warningLine = 'WARNING|LINT|AVOID_PRINT|/home/runner/work/repo/lib/foo.dart|10|2|10|Avoid print calls.';
const infoLine = 'INFO|LINT|UNUSED_IMPORT|/home/runner/work/repo/lib/bar.dart|5|1|5|Unused import.';
const makeOptions = (failOn) => ({ failOn });
describe('ParsedLine', () => {
    beforeAll(() => {
        process.env.GITHUB_WORKSPACE = GITHUB_WORKSPACE;
    });
    it('parses error line correctly', () => {
        const pl = new ParsedLine({ line: baseLine }, makeOptions(FailOnEnum.Error));
    ...
    it('parses warning line correctly', () => {
        const pl = new ParsedLine({ line: warningLine }, makeOptions(FailOnEnum.Warning));
    ...
    it('returns correct emoji', () => {
        expect(new ParsedLine({ line: baseLine }, makeOptions(FailOnEnum.Error)).emoji).toBe(':bangbang:');
        expect(new ParsedLine({ line: warningLine }, makeOptions(FailOnEnum.Warning))
            .emoji).toBe(':warning:');
        expect(new ParsedLine({ line: infoLine }, makeOptions(FailOnEnum.Info)).emoji).toBe(':eyes:');
    });
    it('returns correct isFail for each failOn', () => {
        expect(new ParsedLine({ line: baseLine }, makeOptions(FailOnEnum.Nothing))
            .isFail).toBe(false);
        expect(new ParsedLine({ line: baseLine }, makeOptions(FailOnEnum.Error)).isFail).toBe(true);
        expect(new ParsedLine({ line: warningLine }, makeOptions(FailOnEnum.Error))
            .isFail).toBe(false);
        expect(new ParsedLine({ line: warningLine }, makeOptions(FailOnEnum.Warning))
            .isFail).toBe(true);
        expect(new ParsedLine({ line: infoLine }, makeOptions(FailOnEnum.Info)).isFail).toBe(true);
    });
    it('returns correct humanReadableString', () => {
        const pl = new ParsedLine({ line: baseLine }, makeOptions(FailOnEnum.Error));
        expect(pl.humanReadableString).toContain('Error - `lib/main.dart`:96:13 - Prefer const with constant constructors. (prefer_const_constructors).');
    });
});
//# sourceMappingURL=ParsedLine.test.js.map